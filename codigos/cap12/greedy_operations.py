...
def main():
    ...
    print(format_string2 % ("Greedy error", re.match( expression2, path_string ).group(1)))

    expression3 = "(/.+?)/" # non-greedy operator expression
    print(format_string2 % ("No error, base only", re.match( expression3, path_string ).group(1)))
